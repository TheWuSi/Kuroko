...
### FR-11 Docker 部署
- **FR-11-1 编排容器化交付**：项目根目录提供标准的 `docker-compose.yml` 配置文件。
- **FR-11-2 GHCR 镜像集成**：预制镜像配置默认指向 GitHub Actions 构建产出的官方 GHCR 镜像。
- **FR-11-3 端口与环境解耦**：
  - 后端服务监听端口支持通过环境变量（`.env`）灵活指定（默认 `8000`）。
  - 前端服务监听端口支持环境变量配置（默认 `3000` 或整合至统一网关）。
  - 持久化数据卷（SQLite 数据库文件、配置存档）独立挂载，保证容器升级数据不丢失。
